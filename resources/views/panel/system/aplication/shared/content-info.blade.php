...
@else
        ...
        <div class="list-group col-sm-5 col-md-4 col-lg-3 margin-clear panel-item" style="position: relative;">
            ...
            <div id="itemsList">
                @foreach ($aplications['all'] as $index => $aplication)
                    ...
                    @if ($aplication->TbAplicacionID == $aplications['one']->TbAplicacionID)
                        <div id="item{{ $aplication->TbAplicacionID }}" class="list-group-item active">
                    ...
                    <div class="form-group col-sm-12 text-right">
                        <label>
                            ...
                            @if (isset($filter['queries']['status']) && $filter['queries']['status'] == 'deleted')
                                @if (!empty($aplications['one']->RegistroCerrado))
                                    <input type="checkbox" name="cto34_close" value="1" data-id="{{ $aplications['one']->getKey() }}" checked disabled>
                                    <br>
                                    <small class="text-muted">Por: {{ auth_permissions_data(Auth::user()['role'])->CTOUsuarioGrupoNombre }}</small>
                                @else
                                    <input type="checkbox" name="cto34_close" value="1" data-id="{{ $aplications['one']->getKey() }}" disabled>
                                @endif
                            @else
                                @if (!empty($aplications['one']->RegistroCerrado))
                                    @if (Auth::user()['role'] < $aplications['one']->RegistroRol || Auth::user()['role'] == 1)
                                        <input type="checkbox" name="cto34_close" value="1" data-id="{{ $aplications['one']->getKey() }}" checked>
                                        <br>
                                        <small class="text-muted">Por: {{ auth_permissions_data(Auth::user()['role'])->CTOUsuarioGrupoNombre }}</small>
                                    @else
                                        <input type="checkbox" name="cto34_close" value="1" data-id="{{ $aplications['one']->getKey() }}" checked disabled>
                                    ...
                                    @endif
                                @else
                                    <input type="checkbox" name="cto34_close" value="1" data-id="{{ $aplications['one']->getKey() }}">
                                @endif
                            @endif
                        </label>
                    </div>
                ...
                <div role="tabpanel" class="tab-pane row" id="comments">
                    <div class="col-sm-12">
                        ...
                        <div class="items-relation">
                            @forelse($aplications['one']->comments as $comment)
    ...
    --}}
    <input type="hidden" name="_recordId" value="{{ $aplications['one']->getKey() }}">
@endif