			...
			<div class="navbar navbar-static-top navbar-inverse margin-bottom--clear">
				<div class="container-fluid">
					<div class="navbar-header">
						<p class="navbar-text">
							{{ $page['title'] }} / {{ $currencies['one']->MonedaNombre }}
						</p>
					</div>
					<div class="navbar-collapse collapse">
						<ul class="nav navbar-nav navbar-right">
							...
							<li>
								<a href="{{ $navigation['base'] }}/save" class="is-tooltip confirm-leave" data-placement="bottom" title="Nueva moneda">
									<span class="fa-stack fa-lg">
										<i class="fa fa-circle fa-stack-2x"></i>
							...
							<li>
								@if ($navigation['from'] == 'info')
									<a href="{{ $navigation['base'] }}/info/{{ $currencies['one']->tbMonedasID }}{{ $navigation['query_string'] }}" class="is-tooltip" data-placement="bottom" title="Cerrar">
										<span class="fa-stack text-danger fa-lg">
											<i class="fa fa-circle fa-stack-2x"></i>
								...
								@elseif ($navigation['from'] == 'search')
									<a href="{{ $navigation['base'] }}/search/info/{{ $currencies['one']->tbMonedasID  }}{{ $navigation['query_string'] }}" class="is-tooltip" data-placement="bottom" title="Cerrar">
										<span class="fa-stack text-danger fa-lg">
											<i class="fa fa-circle fa-stack-2x"></i>
											<i class="fa fa-times fa-stack-1x fa-inverse"></i>
										</span>
								...
								@elseif ($navigation['from'] == 'filter')
									<a href="{{ $navigation['base'] }}/filter/info/{{ $currencies['one']->tbMonedasID  }}{{ $navigation['query_string'] }}" class="is-tooltip" data-placement="bottom" title="Cerrar">
										<span class="fa-stack text-danger fa-lg">
											<i class="fa fa-circle fa-stack-2x"></i>
											<i class="fa fa-times fa-stack-1x fa-inverse"></i>
										</span>
					...
					<div class="panel-body padding-clear">
						<div class="list-group col-sm-5 col-md-4 col-lg-3 margin-clear panel-item" style="position: relative;">
							...
							@foreach ($currencies['all'] as $index => $currency)
								<a href="{{ $navigation['base'] }}/info/{{ $currency->tbMonedasID  }}{{ ($navigation['page'] > 0) ? '?page='.$navigation['page'] : '' }}" class="list-group-item {{ ($currency->tbMonedasID == $currencies['one']->tbMonedasID) ? 'active' : '' }}">
									<h4 class="list-group-item-heading">{{ $currency->MonedaNombre }}</h4>
									<p class="small {{ ($currency->tbMonedasID == $currencies['one']->tbMonedasID) ? '' : 'text-muted' }}">
						...
						<div>
							<div class="col-sm-7 col-md-8 col-lg-9 panel-item padding-left--clear padding-right--clear" style="position: relative;">
                                ...
                                @include('panel.system.currency.shared.delete-form', [
                                        'title' => 'Moneda',
                                        'name' => $currencies['one']->MonedaNombre,
                                        'id' => $currencies['one']->tbMonedasID,
                                    ])
								...
								<form id="updateForm" action="{{ $navigation['base'] }}/action/update" method="POST" accept-charset="utf-8" class="col-sm-12">
									...
									<input type="hidden" name="cto34_id" value="{{ $currencies['one']->tbMonedasID }}">
