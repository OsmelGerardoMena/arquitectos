            ...
            <div class="tab-content col-sm-12 margin-bottom--20">
                <div role="tabpanel" class="tab-pane active row padding-top--5" id="general">
                    <div class="col-sm-8">
                        ...
                        <p class="helper-text">{{ $currencies['one']->MonedaNombre }}</p>
                    </div>
                    <div class="clearfix"></div>
                    <div class="col-sm-4">
                        <label class="form-label-full">Abreviatura</label>
                        <p class="helper-text">{{ $currencies['one']->MonedaAbreviatura }}</p>
                    </div>
                    <div class="col-sm-4">
                        <label class="form-label-full">Simbolo</label>
                        <p class="helper-text">{{ $currencies['one']->MonedaSimbolo }}</p>
                    </div>
                    <div class="col-sm-4">
                        <label class="form-label-full">Fracción</label>
                        <p class="helper-text">{{ $currencies['one']->MonedaFraccion }}</p>
                    </div>
<!--                     <div class="form-group col-sm-12 text-right">
                        <label>
                            Cerrado:
                            @if (isset($filter['queries']['status']) && $filter['queries']['status'] == 'deleted')
                                @if (!empty($currencies['one']->RegistroCerrado))
                                    <input type="checkbox" name="cto34_close" value="1" data-id="{{ $currencies['one']->getKey() }}" checked disabled>
                                    <br>
                                    <small class="text-muted">Por: {{ auth_permissions_data(Auth::user()['role'])->CTOUsuarioGrupoNombre }}</small>
                                @else
                                    <input type="checkbox" name="cto34_close" value="1" data-id="{{ $currencies['one']->getKey() }}" disabled>
                                @endif
                            @else
                                @if (!empty($currencies['one']->RegistroCerrado))
                                    @if (Auth::user()['role'] < $currencies['one']->RegistroRol || Auth::user()['role'] == 1)
                                        <input type="checkbox" name="cto34_close" value="1" data-id="{{ $currencies['one']->getKey() }}" checked>
                                        <br>
                                        <small class="text-muted">Por: {{ auth_permissions_data(Auth::user()['role'])->CTOUsuarioGrupoNombre }}</small>
                                    @else
                                        <input type="checkbox" name="cto34_close" value="1" data-id="{{ $currencies['one']->getKey() }}" checked disabled>
                                    ...
                                    @endif
                                @else
                                    <input type="checkbox" name="cto34_close" value="1" data-id="{{ $currencies['one']->getKey() }}">
                                @endif
                            @endif
                        </label>
                    </div> -->
                ...
                <div role="tabpanel" class="tab-pane row" id="comments">
                    <div class="col-sm-12">
                        ...
                        <div class="items-relation">
                            @forelse($currencies['one']->comments as $comment)
    ...
    --}}
    <input type="hidden" name="_recordId" value="{{ $currencies['one']->getKey() }}">
@endif