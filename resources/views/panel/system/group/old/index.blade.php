			...
			<div class="navbar navbar-static-top navbar-inverse margin-bottom--clear">
				<div class="container-fluid">
					<div class="navbar-header">
						<p class="navbar-text">
							{{ $page['title'] }} {{ (count($groups['all']) > 0) ? '/ '.$groups['all'][0]->DirGrupoNombre : '' }}
						</p>
					</div>
					<div class="navbar-collapse collapse">
						<ul class="nav navbar-nav navbar-right">
							...
							<li>
								<a href="{{ $navigation['base'] }}/update/{{ $groups['all'][0]->tbDirGrupoID }}{{ ($navigation['page'] > 0) ? '?page='.$navigation['page'] : '' }}" class="is-tooltip" data-placement="bottom" title="Editar">
									<span class="fa-stack fa-lg">
										<i class="fa fa-circle fa-stack-2x"></i>
						...
						<div class="panel-body padding-clear">
							<div class="list-group col-sm-5 col-md-4 col-lg-3 margin-clear panel-item" style="position: relative;">
								<div class="list-group-item">
									<form action="{{ $navigation['base'] }}/search" method="get">
										<div class="input-group">
											<input name="q" type="text" class="form-control form-control-plain" placeholder="Nombre de grupo">
								...
								@foreach ($groups['all'] as $index => $group)
									...
									@if ($index == 0)
										<a href="{{ $navigation['base'] }}/info/{{ $group->tbDirGrupoID  }}{{ ($navigation['page'] > 0) ? '?page='.$navigation['page'] : '' }}" class="list-group-item active">
									...
									<a href="{{ $navigation['base'] }}/info/{{ $group->tbDirGrupoID  }}{{ ($navigation['page'] > 0) ? '?page='.$navigation['page'] : '' }}" class="list-group-item">
										<h4 class="list-group-item-heading">{{ $group->DirGrupoNombre }}</h4>
							...
							<div>
								<div class="col-sm-7 col-md-8 col-lg-9 panel-item padding-left--clear padding-right--clear" style="position: relative;">
									...
									@include('panel.system.currency.shared.delete-form', [
										'title' => 'Grupo',
										'name' => $groups['all'][0]->DirGrupoNombre,
										'id' => $groups['all'][0]->tbDirGrupoID,
                                	])
									<h4 class="text-muted panel-section-title">Grupo</h4>
									<div class="col-sm-12 margin-bottom--20">
										<div class="col-sm-4">
											<label class="form-label-full">ID</label>
											<p class="helper-text">{{ $groups['all'][0]->tbDirGrupoID }}</p>
										</div>
										<div class="col-sm-8">
											<label class="form-label-full">Nombre</label>
											<p class="helper-text">{{ $groups['all'][0]->DirGrupoNombre }}</p>
										</div>
										<div class="col-sm-12">
											<label class="form-label-full">Descripción</label>
											<p class="helper-text">{{ $groups['all'][0]->DirGrupoDescripcion }}</p>
										</div>
									</div>
								</div>
							</div>
