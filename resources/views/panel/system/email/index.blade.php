				...
				<div class="panel panel-default panel-section bg-white">
					...
					<div class="col-sm-4">
						...
						@include('shared.nav-actions-info',
                            [ 'model' =>
                                [
                                    'count' => $emails['all']->count(),
                                    'id' => ($emails['all']->count() > 0) ? $emails['one']->getKey() : 0,
                                    'inactive' => ($emails['all']->count() > 0) ? $emails['one']->RegistroInactivo : 0,
                                    'closed' => ($emails['all']->count() > 0) ? $emails['one']->RegistroCerrado : 0
                                ]
                            ]
                        )
					</div>
                    ...
                    --}}
					@include('panel.system.email.shared.content-info')
				</div>
			</div>
		</div>
	</div>
	...
	<script>
        (function() {
            ...
            app.closeRecord({
                ...
                table: "{{  ($emails['all']->count() > 0) ? $emails['one']->getTable() : '' }}",
                tableId: "{{ ($emails['all']->count() > 0) ? $emails['one']->getKeyName() : '' }}"
            });
        })();
	</script>
@endpush