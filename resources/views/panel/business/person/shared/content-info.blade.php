...
@else
                    ...
                    <div class="form-group col-sm-12 text-right">
                        <label>
                            ...
                            @if (isset($filter['queries']['status']) && $filter['queries']['status'] == 'deleted')
                                @if (!empty($arancel['one']->RegistroCerrado))
                                    <input type="checkbox" name="cto34_close" value="1" data-id="{{ $arancel['one']->getKey() }}" checked disabled>
                                    <br>
                                    <small class="text-muted">Por: {{ auth_permissions_data(Auth::user()['role'])->CTOUsuarioGrupoNombre }}</small>
                                @else
                                    <input type="checkbox" name="cto34_close" value="1" data-id="{{ $arancel['one']->getKey() }}" disabled>
                                @endif
                            @else
                                @if (!empty($arancel['one']->RegistroCerrado))
                                    @if (Auth::user()['role'] < $arancel['one']->RegistroRol || Auth::user()['role'] == 1)
                                        <input type="checkbox" name="cto34_close" value="1" data-id="{{ $arancel['one']->getKey() }}" checked>
                                        <br>
                                        <small class="text-muted">Por: {{ auth_permissions_data(Auth::user()['role'])->CTOUsuarioGrupoNombre }}</small>
                                    @else
                                        <input type="checkbox" name="cto34_close" value="1" data-id="{{ $arancel['one']->getKey() }}" checked disabled>
                                    ...
                                    @endif
                                @else
                                    <input type="checkbox" name="cto34_close" value="1" data-id="{{ $arancel['one']->getKey() }}">
                                @endif
                            @endif
                        </label>
                    </div>
                ...
                <div role="tabpanel" class="tab-pane row" id="comments">
                    <div class="col-sm-12">
                        ...
                        <div class="items-relation">
                            @forelse($arancel['one']->comments as $comment)
    ...
    --}}
    <input type="hidden" name="_recordId" value="{{ $arancel['one']->getKey() }}">
@endif