...
@else
        ...
        <div class="col-sm-7 col-md-8 col-lg-9 panel-item padding-left--clear padding-right--clear" style="position: relative;">
            ...
            <div class="tab-content col-sm-12 margin-bottom--20">
                <div role="tabpanel" class="tab-pane active row padding-top--5" id="general">
                    ...
                    <div class="form-group col-sm-3 col-sm-offset-6 text-right">
                        <label>{{ $groups['one']->RegistroInactivo == 1 ?'Inactivo':'Activo' }}</label>
                    </div>
                    <div class="form-group col-sm-3 text-right">
                        <label>
                            Cerrado:
                            @if (isset($filter['queries']['status']) && $filter['queries']['status'] == 'deleted')
                                @if (!empty($groups['one']->RegistroCerrado))
                                    <input type="checkbox" name="cto34_close" value="1" data-id="{{ $groups['one']->getKey() }}" checked disabled>
                                    <br>
                                    <small class="text-muted">Por: {{ auth_permissions_data(Auth::user()['role'])->CTOUsuarioGrupoNombre }}</small>
                                @else
                                    <input type="checkbox" name="cto34_close" value="1" data-id="{{ $groups['one']->getKey() }}" disabled>
                                @endif
                            @else
                                @if (!empty($groups['one']->RegistroCerrado))
                                    @if (Auth::user()['role'] < $groups['one']->RegistroRol || Auth::user()['role'] == 1)
                                        <input type="checkbox" name="cto34_close" value="1" data-id="{{ $groups['one']->getKey() }}" checked>
                                        <br>
                                        <small class="text-muted">Por: {{ auth_permissions_data(Auth::user()['role'])->CTOUsuarioGrupoNombre }}</small>
                                    @else
                                        <input type="checkbox" name="cto34_close" value="1" data-id="{{ $groups['one']->getKey() }}" checked disabled>
                                    ...
                                    @endif
                                @else
                                    <input type="checkbox" name="cto34_close" value="1" data-id="{{ $groups['one']->getKey() }}">
                                @endif
                            @endif
                        </label>
                    </div>
                ...
                <div role="tabpanel" class="tab-pane row" id="comments">
                    <div class="col-sm-12">
                        ...
                        <div class="items-relation">
                            @forelse($groups['one']->comments as $comment)
    ...
    --}}
    <input type="hidden" name="_recordId" value="{{ $groups['one']->getKey() }}">
@endif