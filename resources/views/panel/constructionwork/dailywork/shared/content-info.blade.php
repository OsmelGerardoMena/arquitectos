            ...
            <div id="itemsList">
                @foreach ($dailywork['all'] as $index => $daily)
                    @if ($daily->tbDiarioID == $dailywork['one']->tbDiarioID)
                        <div id="item{{ $daily->tbDiarioID }}" class="list-group-item active">
                            <h4 class="list-group-item-heading">
                                {{ $daily->DiarioFolio }} |
                                {{ $daily->DiarioAsunto }}
                            </h4>
                        ...
                        @continue
                    @endif
                    <a id="item{{ $daily->tbDiarioID }}" href="{{ $navigation['base'].$navigation['section'] }}/{{ $daily->tbDiarioID  }}{{ !empty($filter['query']) ? '?'.$filter['query'] : '' }}" class="list-group-item">
                        <h4 class="list-group-item-heading">
                            {{ $daily->DiarioFolio }} |
                            {{ $daily->DiarioAsunto }}
                        </h4>
                ...
                <div role="tabpanel" class="tab-pane row" id="comments">
                    <div class="col-sm-12">
                        ...
                        <div class="items-relation">
                            @forelse($dailywork['one']->comments as $comment)
                                <a href="#" data-id="{{ $comment->tbComentarioID  }}" data-toggle="modal" data-author="{{ $comment->tbCTOUsuarioID_Comentario }}" data-user="{{ Auth::id() }}" data-target="#showCommentModal" class="list-group-item" style="border-bottom: 1px solid #ddd">
                                    <div class="row">
                                        <div class="col-sm-8">
                                            <p id="commentDescription" class="help-block help-block--textarea" style="background-color: #f5f5f5; border: 1px solid #f5f5f5">{{ str_limit($comment->Comentario,240) }} </p>
                                        </div>
                                        <div class="col-sm-4">
