            ...
            <div class="tab-content col-sm-12 margin-bottom--20">
                <div role="tabpanel" class="tab-pane active row padding-top--5" id="general">
                    ...
                    <div class="form-group col-sm-6">
                        <label>Correo</label>
                        <p class="help-block">{{ $emails['one']->DirEmailCorreoE }}</p>  
                    </div>

<!--                     <div class="form-group col-sm-6">
                        <label>Comentarios</label>
                        <p class="help-block">{{ $emails['one']->DirEmailComentarios }}</p>  
                    </div> -->
                    <div class="form-group col-sm-6">
                        <label>Estatus</label>
                        <p class="help-block">{{ $emails['one']->RegistroInactivo == 1 ?'Inactivo':'Activo' }}</p>
                    </div>
                    <div class="form-group col-sm-12 text-right">
                        <label>
                            Cerrado:
                            @if (isset($filter['queries']['status']) && $filter['queries']['status'] == 'deleted')
                                @if (!empty($emails['one']->RegistroCerrado))
                                    <input type="checkbox" name="cto34_close" value="1" data-id="{{ $emails['one']->getKey() }}" checked disabled>
                                    <br>
                                    <small class="text-muted">Por: {{ auth_permissions_data(Auth::user()['role'])->CTOUsuarioGrupoNombre }}</small>
                                @else
                                    <input type="checkbox" name="cto34_close" value="1" data-id="{{ $emails['one']->getKey() }}" disabled>
                                @endif
                            @else
                                @if (!empty($emails['one']->RegistroCerrado))
                                    @if (Auth::user()['role'] < $emails['one']->RegistroRol || Auth::user()['role'] == 1)
                                        <input type="checkbox" name="cto34_close" value="1" data-id="{{ $emails['one']->getKey() }}" checked>
                                        <br>
                                        <small class="text-muted">Por: {{ auth_permissions_data(Auth::user()['role'])->CTOUsuarioGrupoNombre }}</small>
                                    @else
                                        <input type="checkbox" name="cto34_close" value="1" data-id="{{ $emails['one']->getKey() }}" checked disabled>
                                    ...
                                    @endif
                                @else
                                    <input type="checkbox" name="cto34_close" value="1" data-id="{{ $emails['one']->getKey() }}">
                                @endif
                            @endif
                        </label>
                    </div>
                ...
                <div role="tabpanel" class="tab-pane row" id="comments">
                    <div class="col-sm-12">
                        ...
                        <div class="items-relation">
                            @forelse($emails['one']->comments as $comment)
    ...
    --}}
    <input type="hidden" name="_recordId" value="{{ $emails['one']->getKey() }}">
@endif